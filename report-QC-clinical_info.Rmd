...
<span style="background-color:greenyellow">`r english::Words(length(about_table$mostly_na))` variables having too many missing values (>80%) were removed.</span>
...
`r english::Words(nrow(about_table$one_value_only))` variables had only one identical value for all samples. 
...
`r english::Words(nrow(about_table$one_value_or_na))` variables had only one identical value or missing for all samples.
...
* `r english::Words(length(about_table$wrong_code$noyes))` variables for Yes / No question was supposed to be coded as `2` and `1`, according to the codebook. 
...
* `r english::Words(length(about_table$wrong_code$noyes_little_to_lot))` variables for 'No', 'Yes, a little', 'Yes, quite a lot', 'Yes, a lot' question was supposed to be coded with natural numbers from `1` to `4`, according to the codebook. 
