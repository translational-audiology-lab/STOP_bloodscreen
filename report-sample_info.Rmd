---
title: "About samples and clinical data"
...
* The results presented in this section are based on the data after QC of the given data. The QC is described thoroughly in the section "QC of clinical data table". 
...
```{r}
#  ANOVA P-value
tmp <- anova(lm(Age ~ `Sample Lab`, data = qns))[['Pr(>F)']][1] %>% signif(3)
```
  ...
  for(ii in vars_in) {
    ...
    if(dim(sub_tbl)[1L] > 1 & dim(sub_tbl)[2L] > 2) {
      sub_tbl %>% 
        ...
        {signif(.$p.value, 3)} %>% 
        sfsmisc:::toLatex.numeric() %>%     # * 10^x format
        cat(" ( Fisher's test P = $", ., "$)\n", sep = "")
    } else {
      cat("\n")
    }
