...
* Two controls were excluded that matched to duplicated two cases included in those 1100 participants.
...
Among them, the data of `r n_distinct(unlist(during_qc$i_all_missing))` samples was note obtained for whole measures of one panel or two.
* The data of `r unique(during_qc$samples_too_many_llod$Panel)` of `r nrow(during_qc$samples_too_many_llod)` sample was excluded
...
m_dist <- lapply(
  pca,
  function(xx) {
    rowSums(scale(xx$x)^2) %>% 
      as_tibble(rownames = "SampleID") %>% 
      mutate(
        pval = pchisq(value, df = ncol(xx$x), lower.tail = FALSE),
      )
  }
)
outliers <- lapply(
  ...
  subset(., sapply(., function(x) length(x) > 0))
```

There was `r n_distinct(unlist(outliers))` obvious outlier.
It was labeled with its `RID` in the PCA plot below.

```{r}
for(ii in names(outliers)) {
  p <- pca_plot(pca[[ii]], "x", qns, "RID") +
    geom_point(aes(color = Tinnitus)) +
    geom_label(
      aes(label = if_else(ID %in% outliers[[ii]], ID, NA_character_)),
      nudge_x = 5,
      na.rm = TRUE
    ) +
    ggtitle(ii)
