...
hits <- filter(assoc_res$covariates[["`Collection Date`"]], adj.P < 0.05) %>%
  split(., recode(sign(.$Estimate), "1" = "inc", "-1" = "dec"))
stopifnot(is.null(hits$inc), nrow(hits$dec) == 1)
```

`r Words(nrow(hits$dec))` protein was found correlated with sample collection date or the age of samples by linear regression.

```{r, fig.width=4, fig.height=3}
show_hit_table(hits$dec)
show_examples(hits$dec$OlinkID, `Collection Date`) +
  geom_point()
```
...
```{r}
is_hit <- map(assoc_res$THI_scores, ~ {.x$qval < 0.05})
```

* `r Words(sum(unlist(is_hit)))` protein was significantly associated with one of those THI scores (_q_ value < 0.05).
...
for(ii in 1:nrow(others)) {
  cat(
    ...
    "protein was significantly associated (_q_ value).\n"
  )
