...
```{r Assoc setup, echo=FALSE, include=FALSE}
## Pre-loaded packages
...
fn <- list(
  ...
  o = list(         # intermediate results
    res = "../cache/assoc_overall_samples.RData"
  )
)
...
load(fn$i$c02)
# confirm no samples without proteomic data in `qns`
...
The smoking status is **missing** for a relatively large proportion of participants
...
```{r Assoc.w.covariates, purl=TRUE, cache=TRUE}
...
Especially, quite a large number of proteins were significantly associated with **age, sex and sample lab**. 
...
In order to take into account dissimilar age distribution of sample labs, the association was tested after adjusting for age.
...
- The **'Adj.P by perm.'** is the adjusted P value by $`r N_PERMUTATION`$ permutation using Westfall and Young's max-T method ([Westfall and Young, 1993](https://www.google.com/books?hl=en&lr=&id=nuQXORVGI1QC&oi=fnd&pg=PR11&dq=resampling-based+multiple+testing+examples+and+methods+for+p-value+adjustment+pdf&ots=XngUGMa2LO&sig=Vr87y-1gzC_Glhz3sB6N9R95MU0)). 
...
- The 'q value' was computed using 'qvalue' (v `r packageVersion("qvalue")`) R package ([Ref](https://projecteuclid.org/euclid.aos/1074290335)) to adjust the 'p value's for the number of hypothesis tests. 

- Those **multiple testing correction** (given as 'Adj.P by perm.' and 'q value') were conducted **per table**. 
...
```{r Assoc.tinnitus.no_adjust, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$no_adjust <- lm_prot_padj_by_max_t(
  "Tinnitus", 
  olink, qns, n_iter = N_PERMUTATION
)
```

* `r Words(sum(out_df$perm_P < 0.05))` protein
was significantly associated with tinnitus (adjusted P by permutation < 0.05). 

```{r}
left_join(out_df, proteins, by = "OlinkID") %>% 
  show_res_p(Protein, "$\\Delta$(Yes - No)" = Estimate, "Adj.P by perm." = perm_P, n = 5)
```

##### Covariates included : **age and sex**

```{r Assoc.tinnitus.age_sex, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$age_sex <- lm_prot_padj_by_max_t(
  "Tinnitus + Age + Sex", 
  olink, qns, n_iter = N_PERMUTATION
)
```
...
##### Covariates included : age, sex, and **sample lab**

```{r Assoc.tinnitus.age_sex_lab, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$age_sex_lab <- lm_prot_padj_by_max_t(
...
##### Covariates included : age, sex, and **smoking**

```{r Assoc.tinnitus.age_sex_smoke, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$age_sex_smoke <- lm_prot_padj_by_max_t(
  "Tinnitus + Age + Sex + Smoking", 
  olink, qns, n_iter = N_PERMUTATION
)
```

* Among `r nrow(proteins)` proteins, none was significantly associated with tinnitus  (adjusted P by permutation < 0.05). 
...
##### Covariates included : age, sex, **sample lab**, and **smoking**

```{r Assoc.tinnitus.age_sex_lab_smoke, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$age_sex_lab_smoke <- lm_prot_padj_by_max_t(
...
##### Covariates included : **hearing problem** (`TSCHQ_26`) as well as age, sex, and smoking

```{r Assoc.tinnitus.age_sex_smoke_hearprob, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$age_sex_smoke_hearprob <- lm_prot_padj_by_max_t(
...
##### Covariates included : age, sex, sample lab, smoking, and hearing problem

```{r Assoc.tinnitus.age_sex_lab_smoke_hearprob, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$age_sex_lab_smoke_hearprob <- lm_prot_padj_by_max_t(
...
```{r Assoc.tinnitus.no_aid_user, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$non_aid_user <- lm_prot_padj_by_max_t(
  "Tinnitus + Age + Sex + `Sample Lab`", 
  olink, qns_non_aids, n_iter = N_PERMUTATION
)
```
...
```{r Assoc.tinnitus.non_aid_user_smoke_hearprob, purl=TRUE, cache=TRUE}
out_df <- assoc_res$tinnitus$non_aid_user_smoke_hearprob <- lm_prot_padj_by_max_t(
  "Tinnitus + Age + Sex + `Sample Lab` + Smoking + TSCHQ_26", 
  olink, qns_non_aids, n_iter = N_PERMUTATION
)
```

* Due to missing smoking and/or hearing problem status, only `r nrow(drop_na(qns_non_aids, Smoking, TSCHQ_26))` samples were included in this analysis.
* `r Words(sum(out_df$perm_P < 0.05))` protein
was significantly associated with tinnitus (adjusted P by permutation < 0.05). 
...
- Stress related variables : `r knitr::combine_words(stress$lab, before = "\x60")`.
...
* Covariates included : **individual stress related variables** as well as age, sex, smoking, and Sample Lab

- `lm(Protein ~ (tinnitus status) + age + sex + smoking + sample lab + "a stress")`


```{r Assoc.tinnitus.stress, purl=TRUE, cache=TRUE}
assoc_res$tinnitus$stress <- lapply(
  stress$lab,
  function(ii) {
    paste0("Tinnitus + Age + Sex + Smoking + `Sample Lab` + `", ii, "`") %>% 
      lm_prot_padj_by_max_t(olink, qns, n_iter = N_PERMUTATION)
  }
)
```
...
for(ii in 1:nrow(stress)) {
  p <- assoc_res$tinnitus$stress[[ii]] %>% 
    left_join(proteins, by = "OlinkID") %>% 
    show_res_p(
      Protein, "$\\Delta$(Yes - No)" = Estimate, "Adj.P by perm." = perm_P,
...
* Covariates included : **all `r nrow(stress)` stress related variables** plus age, sex, smoking, and Sample Lab

- `lm(Protein ~ (tinnitus status) + age + sex + smoking + sample lab + stress 1 + stress 2 + ... + stress i)`

```{r Assoc.tinnitus.stress_all, purl=TRUE, cache=TRUE}
assoc_res$tinnitus$stress_all <- paste0("`", stress$lab, "`", collapse = " + ") %>% 
  paste("Tinnitus + Age + Sex + Smoking + `Sample Lab` +", .) %>% 
  lm_prot_padj_by_max_t(olink, qns, n_iter = N_PERMUTATION)
```

```{r}
assoc_res$tinnitus$stress_all %>% 
  left_join(proteins, by = "OlinkID") %>% 
  show_res_p(
    Protein, "$\\Delta$(Yes - No)" = Estimate, "Adj.P by perm." = perm_P,
...
including age, sex, smoking, sample lab, and tinnitus status.

- `lm(Protein ~ (a stress variable) + age + sex + smoking + sample lab + tinnitus status)`

```{r Assoc.tinnitus, purl=TRUE, cache=TRUE}
# Association tests
assoc_res$stress <- lapply(
  stress$lab,
  function(ii) {
    paste0("`", ii, "` + Age + Sex + Smoking + `Sample Lab` + Tinnitus") %>% 
      lm_prot_padj_by_max_t(olink, qns, n_iter = N_PERMUTATION)
  }
)
```
...
* `r Words(sum(unlist(map(assoc_res$stress, ~ {.x$perm_P < 0.05}))))` protein
was significantly associated with one of stress variables (adjusted P by permutation < 0.05). 
...
for(ii in 1:nrow(stress)) {
  p <- assoc_res$stress[[ii]] %>% 
    left_join(proteins, by = "OlinkID") %>% 
    show_res_p(
      Protein, Estimate, "Adj.P by perm." = perm_P,
      caption = paste0(stress$desc[ii], " (", stress$lab[ii], ")")
    )
  print(p)
} 

tmp <- map(assoc_res$stress, ~ {.x$perm_P < 0.05})
for(ii in 1:nrow(stress)) {
  if(any(tmp[[ii]])) {
    ...
    p <- olink_qns %>% 
      ...
      ggplot() +
      aes_string(x = stress$lab[ii], y = "NPX") +
      facet_wrap(~ Assay) +
...
including age, sex, smoking, and sample lab.

- `lm(Protein ~ (hearing problem) + age + sex + smoking + sample lab`

```{r Assoc.hearing_problem, purl=TRUE, cache=TRUE}
# Association tests
f <- NPX ~ TSCHQ_26 + Age + Sex + Smoking + `Sample Lab`
...
- `aov(Protein ~ age + sex + smoking + sample lab + (a subtype variable))`
...
When the _p_ value is significant, the association detected by ANOVA is doubtful. 

```{r Assoc.subtype.cat, purl=TRUE, cache=TRUE}
# Association tests
assoc_res$subtype$cat <- sapply(
  tinnitus_subtypes$cat,
  function(ii) {
    f <- formula(paste("NPX ~ Age + Sex + Smoking + `Sample Lab` +", ii))
    f_b <- formula(paste("NPX ~", ii))  # formula for Bartlett test
    olink_qns %>%
      select(OlinkID, Assay, all_of(all.vars(f))) %>% # due to too wide table
      nest(data = -c(OlinkID, Assay)) %>%
      add_column(map_dfr(.$data, ~ aov_out_1line(f, .x))) %>%
      mutate(`Bartlett P` = map_dbl(data, ~ bartlett.test(f_b, .x)$p.value)) %>% 
      select(-data) %>% 
      mutate(qval = qvalue::qvalue(Pval)$qvalues)
  },
  simplify = FALSE
...
- `r Words(length(tinnitus_subtypes$qty))` categorical variables were tested (
...
- `lm(Protein ~ (a subtype variable) + age + sex + smoking + sample lab)`

```{r Assoc.subtype.qty, purl=TRUE, cache=TRUE}
# Association tests
assoc_res$subtype$qty <- sapply(
  tinnitus_subtypes$qty,
  function(ii) {
    f <- formula(paste("NPX ~", ii, "+ Age + Sex + Smoking + `Sample Lab`"))
    olink_qns %>%
      select(OlinkID, Assay, all_of(all.vars(f))) %>% # due to too wide table
      nest(data = -c(OlinkID, Assay)) %>%
      add_column(map_dfr(.$data, ~ lm_out_1line(f, .x))) %>%
...
- There were `r length(tinnitus_handi_inv)` tinnitus handicap inventory variables, `THI_??`.
...
- `lm(Protein ~ (a THI score) + age + sex + smoking + sample lab)`

- Due to missing information, `r nrow(drop_na(qns, Smoking, all_of(tinnitus_handi_inv)))` samples were analyzed here.

```{r Assoc.THI_scores, purl=TRUE, cache=TRUE}
# Association tests
assoc_res$THI_scores <- lapply(
  tinnitus_handi_inv,
  function(ii) {
    rhs <- paste(ii, "+ Age + Sex + Smoking + `Sample Lab`")
    lm_prot_padj_by_max_t(rhs, olink, qns, n_iter = N_PERMUTATION)
  }
) %>% 
  `names<-`(tinnitus_handi_inv)
```

##### Results

```{r}
is_hit <- map(assoc_res$THI_scores, ~ {.x$perm_P < 0.05})
```

* `r Words(sum(unlist(is_hit)))` proteins were significantly associated with one of those THI scores (adjusted P by permutation < 0.05).

```{r, fig.height=9}
for(ii in tinnitus_handi_inv) {
  if(any(is_hit[[ii]])) {
    qn <- paste(ii, ":", codes$Label[codes$Variable == ii])
    
    hits <- assoc_res$THI_scores[[ii]] %>% 
      filter(is_hit[[ii]]) %>% 
      arrange(Pval)

    hits %>% 
      show_res_p(Protein = Assay, "Trend Yes-Sometimes-No" = Estimate, 
                 "Adj.P by perm." = perm_P, caption = qn, n = Inf) %>% 
      print()
...
- `lm(Protein ~ (THI total score) + age + sex + smoking + sample lab)`

- Due to missing information, `r nrow(drop_na(qns, Smoking, all_of("THI Total score")))` samples were analyzed here.

##### Numeric score

```{r Assoc.THI_total_score.num, purl=TRUE, cache=TRUE}
f <- NPX ~ `THI Total score` + Age + Sex + Smoking + `Sample Lab`
# Association tests
...
```{r Assoc.THI_total_score.ord, purl=TRUE, cache=TRUE}
f <- NPX ~ THI.Total.score.cat + Age + Sex + Smoking + `Sample Lab`
# Association tests
...
- Linear regression model for the association between one protein and one of selected variables, including age, sex, smoking, and sample lab.
If both cases and controls were included, tinnitus status was also included as an adjusting factor.

- `lm(Protein ~ (a variable) + age + sex + smoking + sample lab + tinnitus)`

```{r Assoc.others, purl=TRUE, cache=TRUE}
others <- tribble(
  ~lab,      ~test,  ~desc,
...
assoc_res$others <- lapply(
  ...
  function(ii) {
    f <- formula(paste(
      "NPX ~", others$lab[ii], "+ Age + Sex + Smoking + `Sample Lab` + Tinnitus"
    ))
    no_na <- olink_qns %>%
...
for(ii in 1:nrow(others)) {
  ...
  p <- if(others$test[ii] == "lm") {
    assoc_res$others[[ii]] %>% 
      show_res_p(Protein = Assay, Estimate, "q value" = qval)
  } else {
    assoc_res$others[[ii]] %>% 
      show_res_p(Protein = Assay, "q value" = qval)
  }
