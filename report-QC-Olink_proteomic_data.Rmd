...
library(tidyverse)
library(kableExtra)
library(ggfortify)  # ggplot2::autoplot for PCA
...
```{r excl.no_clinic_info, purl=TRUE}
...
* The number of samples analyzed by Olink assays and for which clinical information is available was `r n_samples_with_info`.
* `r english::Words(n_distinct(missing_tbl$SampleID))` samples were found to contain missing values. 
The whole measures of one or two panels of `r n_distinct(unlist(i_all_missing))` samples were missing.
* Majority of **`r nrow(samples_too_many_llod)` sample**'s protein values in the `r unique(samples_too_many_llod$Panel)` were below LLOD. The data was **removed**.
* The >50% values of **`r length(i_assys_too_many_llod)` proteins** were below LLOD, the data of which were **removed**.
