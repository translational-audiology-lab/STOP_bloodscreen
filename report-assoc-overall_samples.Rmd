...
N_PERMUTATION <- ncol(resample_t_res[["overall"]][[1]][[1]]) - 1
...
Hearing problem data (`TSCHQ_26`) is **missing** for a relatively large proportion of participants
...
```{r}
out_df <- assoc_res$tinnitus[["Tinnitus + Age + Sex"]]
```
...
```{r}
out_df <- assoc_res$tinnitus[["Tinnitus + Age + Sex + `Sample Lab`"]]
```
...
```{r}
out_df <- assoc_res$tinnitus[["Tinnitus + Age + Sex + Smoking"]]
```
...
```{r}
out_df <- assoc_res$tinnitus[["Tinnitus + Age + Sex + `Sample Lab` + Smoking"]]
```
...
```{r}
out_df <- assoc_res$tinnitus[["Tinnitus + Age + Sex + Smoking + TSCHQ_26"]]
```
...
```{r}
out_df <- assoc_res$tinnitus[["Tinnitus + Age + Sex + `Sample Lab` + Smoking + TSCHQ_26"]]
```
...
assoc_res$non_aid_user <- map(
  ...
  ~ lm_prot_padj_by_max_t(.x, olink, qns_non_aids, pos = 1L)
)
```
...
```{r}
out_df <- assoc_res$non_aid_user[["Tinnitus + Age + Sex + `Sample Lab`"]]
```
...
```{r}
out_df <- assoc_res$non_aid_user[["Tinnitus + Age + Sex + `Sample Lab` + Smoking + TSCHQ_26"]]
```
...
- Stress related variables : `r knitr::combine_words(qsets$stress$with_q)`.

- Tests for association between tinnitus and individual proteins

- Due to missing information, `r nrow(drop_na(qns, Smoking, all_of(qsets$stress$lab)))` samples were analyzed here.
...
```{r}
for(ii in 1:nrow(qsets$stress)) {
  rhs <- paste("Tinnitus + Age + Sex + Smoking + `Sample Lab` +", qsets$stress$with_q[ii])
  print(show_delta_P_perm(
    assoc_res$tinnitus[[rhs]], proteins,
    caption = paste0(qsets$stress$desc[ii], " (", qsets$stress$lab[ii], ")")
  ))
}
```

##### **All `r words(nrow(qsets$stress))` stress related variables**

* Covariates included : **all `r nrow(qsets$stress)` stress related variables** plus age, sex, smoking, and Sample Lab

- `lm(Protein ~ (tinnitus status) + age + sex + smoking + sample lab + stress 1 + stress 2 + ... + stress i)`

```{r}
rhs <- paste(qsets$stress$with_q, collapse = " + ") %>% 
  paste("Tinnitus + Age + Sex + Smoking + `Sample Lab` +", .)
print(show_delta_P_perm(
  assoc_res$tinnitus[[rhs]], proteins,
  caption = paste("All", nrow(qsets$stress), "stress related variables")
))
```
...
```{r assoc_res.overall.stress, purl=TRUE, cache=TRUE}
stress_rhs <- map_chr(
  1:nrow(qsets$stress),
  ~ paste("Tinnitus + Age + Sex + Smoking + `Sample Lab` +", qsets$stress$with_q[ii])
)
# names(resample_t_res$overall[qsets$stress_rhs[1]][[1]])
# [1] "TinnitusYes, always"   "Age"                   "SexFemale"            
# [4] "SmokingCurrent smoker" "SmokingEx-smoker"      "`Sample Lab`Stureplan"
# [7] "`Sample Lab`Umeå"      "`PSQ Total score`" 
assoc_res$stress <- map(
  resample_t_res$overall[stress_rhs],
  ~ lm_prot_padj_by_max_t(.x, olink, qns, pos = 8L)
)
```
...
for(ii in 1:nrow(qsets$stress)) {
  if(any(tmp[[ii]])) {
    ...
    p <- olink_qns %>% 
      ...
      drop_na(NPX, Assay, all_of(qsets$stress$lab[ii])) %>% 
      ggplot() +
      aes_string(x = qsets$stress$with_q[ii], y = "NPX") +
      facet_wrap(~ Assay) +
      ggtitle(paste0(qsets$stress$desc[ii], " (", qsets$stress$lab[ii], ")"))
    if(n_distinct(olink_qns[[qsets$stress$lab[ii]]]) == 2) {
...
```{r assoc_res.overall.hearing_problem, purl=TRUE, cache=TRUE}
# Association tests
assoc_res$hearing_problem <- olink_qns %>%
  nest(data = -c(OlinkID, Assay)) %>%
  add_column(map_dfr(.$data, ~ lm_out_1line(
    # to avoid too large cache
    NPX ~ TSCHQ_26 + Age + Sex + Smoking + `Sample Lab`, .x
  ))) %>%
  select(-data) %>% 
  mutate(qval = qvalue::qvalue(Pval)$qvalues)
```
...
- `r Words(length(qsets$tinnitus_subtypes$cat))` categorical variables were tested (
`r knitr::combine_words(qsets$tinnitus_subtypes$cat, before = '\x60')`).
...
```{r}
for(ii in qsets$tinnitus_subtypes$cat) {
  if(any(is_hit[[ii]])) {
...
- `r Words(length(qsets$tinnitus_subtypes$qty))` continuous variables were tested (
`r knitr::combine_words(qsets$tinnitus_subtypes$qty, before = '\x60')`).
...
```{r}
for(ii in qsets$tinnitus_subtypes$qty) {
  if(any(is_hit[[ii]])) {
    qn <- paste(ii, ":", codes$Label[codes$Variable == ii])
...
stopifnot(all(map_lgl(qsets$THI_scores, ~ is.ordered(qns[[.x]]))))
# Confirm the number of included samples is constant
tmp <- drop_na(qns, Smoking)
tmp <- map_int(qsets$THI_scores, ~ sum(!is.na(tmp[[.x]])))
stopifnot(all(tmp == tmp[1]))
```

- There were `r length(qsets$THI_scores)` tinnitus handicap inventory (THI) variables, `THI_??`.
...
- The _p_ values were adjusted for all tested proteins and all `r length(qsets$THI_scores)` THI variables.

- Due to missing information, `r nrow(drop_na(qns, Smoking, all_of(qsets$THI_scores)))` samples were analyzed here.
...
assoc_res$THI_scores <- foreach(
  ii = iterators::iter(qsets$THI_scores),
  .inorder = FALSE,
  ...
  olink_qns %>%
    ...
    add_column(map_dfr(.$data, ~ lm_out_1line(
      # to avoid too large cache
      formula(paste("NPX ~", ii, "+ Age + Sex + Smoking + `Sample Lab`")), 
      .x
    ))) %>%
    select(-data)
} %>% 
...
out_df <- assoc_res$THI_total_score$num <- olink_qns %>%
  ...
  add_column(map_dfr(.$data, ~ lm_out_1line(
    # to avoid too large cache
    NPX ~ `THI Total score` + Age + Sex + Smoking + `Sample Lab`, .x
  ))) %>%
  select(-data) %>% 
  mutate(qval = qvalue::qvalue(Pval)$qvalues)
```
...
out_df <- assoc_res$THI_total_score$ord <- olink_qns %>% 
  ...
  add_column(map_dfr(.$data, ~ lm_out_1line(
    # to avoid too large cache
    NPX ~ THI.Total.score.cat + Age + Sex + Smoking + `Sample Lab`, .x
  ))) %>%
  select(-data) %>% 
  mutate(qval = qvalue::qvalue(Pval)$qvalues)
```
