...
fn <- list(
  ...
  o = list(         # intermediate results
    res = "../cache/assoc-overall_samples.RData"
  )
)
...
for(ii in 1:nrow(stress)) {
  lab <- stress$lab[ii]
  desc <- stress$desc[ii]
  rhs <- paste0("Tinnitus + Age + Sex + Smoking + `Sample Lab` + `", lab, "`")
  print(show_delta_P_perm(
    assoc_res$tinnitus[[rhs]], proteins,
    caption = paste0(desc, " (", lab, ")")
  ))
}
```
...
```{r}
rhs <- paste0("`", stress$lab, "`", collapse = " + ") %>% 
...
stress_rhs <- map_chr(
  1:nrow(stress),
  ~ paste0("Tinnitus + Age + Sex + Smoking + `Sample Lab` + `", stress$lab[.x], "`")
)
# names(resample_t_res$overall[stress_rhs[1]][[1]])
