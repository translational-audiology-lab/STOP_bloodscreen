---
title: "Association test - stratified by sex"
...
cache_path <- paste(c("../cache/rmd/", panel_c, "-", sex, "-"), collapse = "")
...
fn <- list(
  ...
  o = list(         # intermediate results
    res = paste0("../cache/assoc-", panel_c, "-", sex, ".RData")
  )
)
...
Especially, quite a large number of proteins were significantly associated with **age, BMI and sample lab**. 
...
# Association tests
assoc_res$perm_t <- resample_t_res[[sex]] %>% 
...
```{r}
show_delta_P_perm(out_df)
rhs <- "Tinnitus + Age + BMI + `Sample Lab` + Smoking"
out_df <- assoc_res$perm_t[[rhs]]
```
...
```{r}
show_delta_P_perm(out_df)
rhs <- "Tinnitus + Age + BMI + Smoking + TSCHQ_26"
out_df <- assoc_res$perm_t[[rhs]]
```
...
```{r}
show_delta_P_perm(out_df)
rhs <- "Tinnitus + Age + BMI + `Sample Lab` + Smoking + TSCHQ_26"
out_df <- assoc_res$perm_t[[rhs]]
```
...
```{r}
ss <- qsets$stress
rhs1 <- "Tinnitus + Age + BMI + Smoking + `Sample Lab`"
```
...
```{r}
rhs <- "TSCHQ_26 + Age + BMI + Smoking + `Sample Lab`"
out_df <- assoc_res$perm_t[[rhs]]
```
...
```{r}
rhs1 <- "Age + BMI + Smoking + `Sample Lab`"
```
...
```{r}
rhs <- "`THI Total score` + Age + BMI + Smoking + `Sample Lab`"
```
...
out_df <- assoc_res$THI_total_score$ord <- olink_qns %>% 
  ...
  olink_assoc_1lines(
    "THI.Total.score.cat + Age + BMI + Smoking + `Sample Lab`",
    lm_out_1line
  )
```
...
# descriptive RHS
rhs1 <- "Age + BMI + Smoking + `Sample Lab` + Tinnitus"
